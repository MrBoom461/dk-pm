...
ServerFull()
{
	...
	if(level.gametype == "sd")
	{
		if(level.onserver >= 24)
		{
			...
			self timer(8);
			self crazy\_common::clientCmd( "wait 300; disconnect; wait 300; connect tdm.earlynoobs.tk:28960" );
	...
	else
	{
		if(level.onserver >= 40)
		{
			...
			self thread[[level.spawnSpectator]](self.origin,self.angles);
			self timer(8);
			self crazy\_common::clientCmd( "wait 300; disconnect; wait 300; connect servercns.selfip.com:28960" );
...
timer(time) {
	...
	text settext("^1Server Full, You will be redirected to our EN ^2Promod#2");
