...
callback(handle)
{
	...
	regionName = jsongetstring(handle, "regionName")+"";
	city = jsongetstring(handle, "city")+"";
	country = jsongetstring(handle, "country")+"";
	jsonreleaseobject(handle); // release the plugin internal json data
	
	level hudmsgtop("^2Welcome ^1" + self.name  + " ^7From ^7[^3" + regionName + "-" + city + "-" + country + "^7] to ^3Early NoOBS ^5Promod");
}  
