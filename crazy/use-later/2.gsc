...
onPlayerConnected()
{
	...
	if(self.pers["sles_killcam_style"] == 2){
		...
		self setClientDvar( "sles_killcamName", "Early NoObs (Digital Camcorder) Kill-Camera" );
	}
	if(self.pers["sles_killcam_style"] == 3){
		...
		self setClientDvar( "sles_killcamName", "Early NoOBs (Fun Sinhala) Kill-Camera" );
	}
	...
	if(self.pers["sles_killcard"] == 0){
		...
		self setClientDvar( "sles_killcardName", "Early NoOBs (Default) Kill-Card" );
...
ToggleBinds()
{
	...
	for(;;)
	{
		...
		{
			if(self.pers["sles_knife_mute"] == 0)
			{
				self setClientDvar( "sles_knife_mute", 1 );
				self setstat(3158,1);
				self.pers["sles_knife_mute"] = 1;
				self quickNotify("^5Knife Music","^2OFF");
			}
			else if (self.pers["sles_knife_mute"] == 1)
			{
				self setClientDvar( "sles_knife_mute", 0 );
				self setstat(3158,0); 
				self.pers["sles_knife_mute"] = 0;
				self quickNotify("^5Knife Music","^3ON");
			}
		}
		if(response == "killCamMusic")
		{
			if(self.pers["sles_kill_mute"] == 0)
			{
				self setClientDvar( "sles_kill_mute", 1 );
				self setstat(3157,1);
				self.pers["sles_kill_mute"] = 1;
				self quickNotify("^5Killcam Music","^2OFF");
			}
			else if (self.pers["sles_kill_mute"] == 1)
			{
				self setClientDvar( "sles_kill_mute", 0 );
				self setstat(3157,0); 
				self.pers["sles_kill_mute"] = 0;
				self quickNotify("^5Killcam Music","^3ON");
			}
		}
		...
		if(response == "sles_killcam_style_menu")
		{
			...
			else if (self.pers["sles_killcam_style"] == 1)
			{
				...
				self setClientDvar( "sles_killCamStyleSample", "killcam3" );
				self setClientDvar( "sles_killcamName", "Early NoOBs (Digital Camcorder) Kill-Camera" );
				//self quickNotify("^5Damage FeedBack : ","^7Early NoOBs (Digital Camcorder) Kill-Camera");
			}
			else if (self.pers["sles_killcam_style"] == 2)
			{
				...
				self setClientDvar( "sles_killCamStyleSample", "killcam4" );
				self setClientDvar( "sles_killcamName", "Early NoOBs (Fun Sinhala) Kill-Camera" );
				//self quickNotify("^5Damage FeedBack : ","^7Early NoOBs (Fun Sinhala) Kill-Camera");
			}
