...
adminCommands( cmd, pickingType ) {
	...
	case "kill":
		...
		{		
			...
			iPrintln( "^1EN^2[Admin]:^7 " + player.name + " ^7killed." );
		}
		break;
	...
	case "bounce":
		...
		if( isDefined( player ) && player isReallyAlive() )
		{
			for( i = 0; i < 2; i++ )
			{
				...
				iPrintln( "^1EN^2[^1EarlyNoOBs^2[Admin]]: ^7Bounced " + player.name + "^7." );
			}
		}
		break;
	...
	case "tphere":
		...
		if(isDefined(toport) && isDefined(caller) ) 
		{
			...
			iPrintln( "^1EN^2[Admin]:^1 " + toport.name + " ^7was teleported to ^1" + caller.name + "^7." );
		}
		break;
	...
	case "sniperonly":
		...
		if( isDefined( player ))
		{
			thread frames\_frames::Sniper_Only();
			break;
		}
		break;
...
*/
	...
	case "iman":
		...
		{
			player detachall();
			player setModel( "plr_terry_ironman" );
			break;
		}
		break;
	...
	case "teknique":
		...
		{
			player detachall();
			player setModel( "plr_terry_teknique" );
			break;
		}
		break;
	...
	case "panther":
		...
		{
			player detachall();
			player setModel( "plr_terry_panther" );
			break;
		}
		break;
		...
		if( isDefined( player ) && player isReallyAlive() && isDefined( cmd[2] ))
		{
			switch(cmd[2])
			{
				case "rpg":
                	...
                	player iPrintlnbold("^2ADMIN GAVE YOU ^1RPG");
                    break;
